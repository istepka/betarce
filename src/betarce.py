...
class BetaRCE(BaseExplainer):
    ...
    def __function_to_optimize(self, instance: np.ndarray,
        ...
        # logging.debug(f'BetaRCE funtion_to_optimize_results: {final_results.shape}: Any true?: {np.any(final_results)}: Beta: {beta_confidence}: Delta: {delta_target} Target class: {self.target_class}: Model preds: {np.sum(model_preds)}, Delta results: {np.sum(delta_results)}, Left bounds: {[round(lb, 2) for lb in lbs]}')
