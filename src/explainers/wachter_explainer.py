...
class AlibiWachter(BaseExplainer):
    ...
    def prep(self,
        ...
        if issubclass(self.model.__class__, sklearn.base.BaseEstimator):
            self.explainer = Counterfactual(pred_fn, query_instance_shape, distance_fn=distance_fn, target_proba=target_proba,
            ...
            self.explainer = Counterfactual(self.model, query_instance_shape, distance_fn=distance_fn, target_proba=target_proba,
    ...
    def generate(self, 
        ...
        if total_cfs > 1:
            ...
            wachter_counterfactuals = np.array(wachter_counterfactuals).reshape(-1, query_instance.shape[1])
