...
class DiceExplainer(BaseExplainer):
    ...
    def prep(
        ...
        assert (
            self.outcome_name in self.dataset.columns
        ), f"outcome_name must be in dataset.columns"
        ...
        dice_model = dice_ml.Model(
            model=self.model, backend="sklearn", func=feature_encoding
        )
    ...
    def generate(
        ...
        **kwargs,
        ...
        final_cf = explanations_object.cf_examples_list[0].final_cfs_df.to_numpy()[0][
            :-1
        ]
