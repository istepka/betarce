...
class TorchModel(MLModel):
    def __init__(
        self, model: nn.Module, data: pd.DataFrame, columns_ohe_order: List[str]
    ) -> None:
        super().__init__(data)
...
class CarlaExplainer(BaseExplainer):
    def __init__(
        ...
        self.model = TorchModel(
            ...
            columns_ohe_order=columns_order_ohe,
        )
    ...
    def prep(self, method_to_use: str, hparams: dict = {}) -> None:
        ...
        match self.method_to_use:
            ...
            case "roar":
                hyperparams = {"seed": 123, "lime_seed": 123, **hparams}
                self.roar_explainer = Roar(self.model, hyperparams)
            case _:
                raise ValueError(
                    f"Method {self.method_to_use} not supported by the explainer"
                )

    def generate(self, query_instance: pd.DataFrame, **kwargs) -> np.ndarray:
