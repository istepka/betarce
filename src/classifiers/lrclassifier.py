...
class LogisticRegressionClassifier(BaseClassifier):
    ...
    def predict_crisp(
        self, x: np.ndarray | pd.DataFrame, threshold=0.5
    ) -> np.ndarray[int]:
        if isinstance(x, pd.DataFrame):
            x = x.values
        if len(x.shape) == 1:
...
def train_logistic_regression(
...
) -> tuple[LogisticRegressionClassifier, callable, callable]:
    """
    ...
    model = LogisticRegressionClassifier(hparams, seed)
...
def train_K_logistic_regressions(
    X_train,
    y_train,
    X_test,
    y_test,
    ...
    for k in range(K):
        ...
        lr = LogisticRegressionClassifier(model_hparams, seed)

        # Train the model
        lr.fit(X_train, y_train)

        # Evaluate the model
        d = lr.evaluate(X_test, y_test)
        accuracy = d["accuracy"]
        recall = d["recall"]
...
def train_K_lrs_in_parallel(
    ...
    results = Parallel(n_jobs=n_jobs)(
        delayed(train_K_logistic_regressions)(
            X_train,
            y_train,
            X_test,
            y_test,
        ...
        for hparams_list, seeds, bootstrap_seeds in zip(
